...
When creating a route, the optional **Link to a KAYA climb** list offers unclaimed climbs matching the selected discipline, colour and grade. The list updates as those fields change, and the selected climb is linked after the route is created. The same picker is available from an unlinked route's detail page. Blank colour or grade fields leave that part unconstrained, the search box narrows the matches further, and the newest sets appear first. Linking adopts the catalog's wall label, overwriting whatever you typed — unlinking does not restore it. One climb can be claimed by at most one route per gym, so a second attempt is refused.
